...
use std::{fs, fmt::Display, sync::Arc};

use crate::story::{story2::*, story_parser::*};



fn main() {

    let head = map_stories();
    println!("{:?}", head.as_ref().unwrap());
    println!("{:?}", &head.as_ref().unwrap().path[0]);
...
fn map_stories() -> Option<StoryBlock2> {
    let file = fs::read_to_string("src\\story.story").unwrap();
    let file_s = &file[..];
