...
use nom::{
    ...
    character::complete::{alphanumeric0, alphanumeric1, multispace0},
    multi::many0,
...
fn story_id(input: &str) -> IResult<&str, String> {
    tuple((multispace0, delimited(tag("["), alphanumeric1, tag("]"))))(input)
        .map(|(next_input, (_, id))| (next_input, id.into()))
}

fn story_text(input: &str) -> IResult<&str, String> {
    take_while(is_not_command)(input).map(|(next_input, parsed)| (next_input, parsed.into()))
}
...
fn story_command(input: &str) -> IResult<&str, Path> {
    tuple((
        ...
        multispace0,
        delimited(tag("{"), take_while(is_not_bracket), tag("}")),
        multispace0,
    ))(input)
    .map(|(left_input, res)| {
...
fn story_block(input: &str) -> IResult<&str, StoryParse> {
    tuple((story_id, story_text, many0(story_command)))(input).map(
        |(left_input, (id, text, command))| {
            (
                left_input,
                StoryParse {
                    id,
